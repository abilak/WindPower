...
model.add(tf.keras.layers.LSTM(128, return_sequences=True, input_shape=(n_steps, 1)))
...
model2.add(tf.keras.layers.LSTM(128, return_sequences=True, input_shape=(n_steps, 1)))
...
final_preds = scaler.inverse_transform(model.predict(X_test) + model2.predict(X_test))
